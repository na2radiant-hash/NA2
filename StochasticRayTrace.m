...
set(gcf, 'Units', 'centimeters', 'Position', [2 2 13 9]);
set(gcf, 'Color', 'w');
...
%% -------------------------
%  Inset: Close-Up View (zoomed around source)
% -------------------------
ax_main = gca;

% Draw dashed zoom-indicator rectangle on main axes
zoom_r = 5;
rectangle('Parent', ax_main, 'Position', [-zoom_r, -zoom_r, 2*zoom_r, 2*zoom_r], ...
    'EdgeColor', [0.25 0.25 0.25], 'LineStyle', '--', 'LineWidth', 0.8);

% Create inset axes on the right side (outside the main data area)
ax_ins = axes('Position', [0.73 0.53 0.23 0.40]);
hold(ax_ins, 'on'); axis(ax_ins, 'equal');

% Lens fill
try
    if ~isempty(lensBody.Vertices)
        plot(ax_ins, lensBody, 'FaceColor', lensFace, 'FaceAlpha', lensAlpha, 'EdgeColor', 'none');
    end
catch; end

% Lens edges
for j_ins = 1:L
    if isempty(final_lens{j_ins}), continue; end
    up_ins = final_lens{j_ins}; lo_ins = up_ins; lo_ins(:,2) = -up_ins(:,2);
    plot(ax_ins, up_ins(:,1), up_ins(:,2), 'Color', edgeColor, 'LineWidth', edgeLW*0.8);
    if draw_mirror
        plot(ax_ins, lo_ins(:,1), lo_ins(:,2), 'Color', edgeColor, 'LineWidth', edgeLW*0.8);
    end
end

% Re-trace rays for inset
for r_ins = 1:N_rays
    P0_ins = origin_global; ac_ins = alphas0(r_ins);
    ni_ins = n1_global;     no_ins = n2_global;
    pth_ins = P0_ins;
    for j_ins = 1:L
        [h_ins, Ph_ins, an_ins] = first_hit_with_layer(P0_ins, ac_ins, j_ins, ni_ins, no_ins, final_lens);
        if ~h_ins, break; end
        pth_ins = [pth_ins; Ph_ins]; %#ok<AGROW>
        P0_ins = Ph_ins; ac_ins = an_ins;
        tmp_ins = ni_ins; ni_ins = no_ins; no_ins = tmp_ins;
    end
    plot(ax_ins, pth_ins(:,1), pth_ins(:,2), '-', 'Color', rayColor, 'LineWidth', rayLW*0.8);
    if draw_mirror
        pm_ins = pth_ins; pm_ins(:,2) = -pm_ins(:,2);
        plot(ax_ins, pm_ins(:,1), pm_ins(:,2), '-', 'Color', rayColor, 'LineWidth', rayLW*0.8);
    end
end

% Source marker
plot(ax_ins, origin_global(1), origin_global(2), 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 4);

xlim(ax_ins, [-zoom_r zoom_r]); ylim(ax_ins, [-zoom_r zoom_r]);
set(ax_ins, 'FontSize', 7, 'FontName', 'Helvetica', 'TickDir', 'out', ...
    'Box', 'on', 'LineWidth', 0.75, 'XTick', [-4 0 4], 'YTick', [-4 0 4]);
xlabel(ax_ins, '$x$ (mm)', 'Interpreter', 'latex', 'FontSize', 7);
ylabel(ax_ins, '$y$ (mm)', 'Interpreter', 'latex', 'FontSize', 7);
axes(ax_main);

set(gcf, 'Renderer', 'painters');
print(gcf, 'ray_trace.pdf', '-dpdf');
